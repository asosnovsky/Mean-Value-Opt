...
for (sector in sector_html_titles[1])
{
  stock_dir <- paste0("data/time_series/2008-2012/",sector)
  stock_list <- gsub(".csv","",dir(stock_dir))
  dir.create("data/filtered_stocks/",showWarnings = FALSE)
    ...
    for(part in 1:length(partitions_year[,1]))
    {
      ...
      merged_data_directory <- paste0("data/filtered_stocks/",year)
      ...
      for (stock in stock_list)
      {
        ...
        if (nrow(data_year) > nrow(date_vector)*.85) #if the stock has more than 85% of time data we keep it
        {
          ...
          for (naInd in which(is.na(merged_data[,ncol(merged_data)]))) {
            if (naInd == 1)
            {
              #if first day is NA skip. 
              next
            }
            else if( last.na+1 == naInd ) 
            {
              merged_data[,ncol(merged_data)][naInd] = merged_data[,ncol(merged_data)][last.na]
            }  
            else 
            {
              merged_data[,ncol(merged_data)][naInd] = data[data[,1] == merged_data$Index[naInd-1],2]
            }
