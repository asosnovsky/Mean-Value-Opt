function [ sharpe, P, Wp, sharpes, mps, risks ] = optimizeSharpPortee( Ret, CoRisk, rfr, portlim, NegOrPos, SUPPMSG )
...
function [] = optimo()
    m0 = limmp(1);
    mk = limmp(2);
    while abs(sharpes(c)-sharpes(c-1))> 1E-25
        ud = [ mk<m0 mk>m0 ];%if mk > m0 then limmp([0 1])=max else limmp([1 0])=min
        m0 = median(limmp);
        mk = median([m0 limmp(ud)]);
                                
        [P, Wp] = optimizePortee( Ret, CoRisk, m0, portlim, NegOrPos, P, Wp  );
        sharpes(c) = sharpe(P,Wp,rfr);
        [P, Wp, lRisk] = optimizePortee( Ret, CoRisk, mk, portlim, NegOrPos, P, Wp  );
